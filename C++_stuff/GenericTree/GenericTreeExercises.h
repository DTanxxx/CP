...
static void treeFactory(GenericTree<int>& tree) {
  ...
  // objective: fill out the tree object so it looks like the above diagram
  ...
  // create the root node ptr
  auto rootptr = tree.createRoot(4);

  // set the children nodes
  auto nodeA = rootptr->addChild(8);
  rootptr->addChild(15);

  auto nodeC = nodeA->addChild(16);
  nodeA->addChild(23);
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  std::vector<TreeNode*> childrenPointers;
  ...
  tree.compress();
  // add root ptr data to vector
  results.push_back(rootNodePtr->data);
  // call recursive function, pass in childrenToCheck vector, results vector, current pointer to traverse through its children
  traverseNode(results, childrenPointers, rootNodePtr);
...
void traverseNode(std::vector<T> &results, std::vector<N*> children, N* currptr) {
  if ((currptr->childrenPtrs).empty()) {
    return;
  }

  using TreeNode = typename GenericTree<N>::TreeNode;
  std::vector<N*> childrenOfCurrptr;

  if (children.empty()) {
    // empty children, so must be root node
    for (int i=0; i<int((currptr->childrenPtrs).size()); i++) {
      // append each first generation nodes to the vector
      childrenOfCurrptr.push_back((currptr->childrenPtrs)[i]);
      results.push_back((currptr->childrenPtrs)[i]->data);
    }

    // traverseNode
    traverseNode(results, childrenOfCurrptr, currptr);
  }
  else {
    for (int i=0; i<int(children.size()); i++) {
      // there is children, so first generation etc...
      for (int j=0; j<int(((children[i])->childrenPtrs).size()); j++) {
        // for each child node in the children vector (so 2nd generation)
        childrenOfCurrptr.push_back(((children[i])->childrenPtrs)[j]);
        results.push_back(((children[i])->childrenPtrs)[j]->data);
      }
    }

    if (childrenOfCurrptr.empty()) {
      return;
    }
