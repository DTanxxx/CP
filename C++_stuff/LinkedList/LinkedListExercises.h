...
void LinkedList<T>::insertOrdered(const T& newData) {
  Node* tailptrBeforeHack = getTailPtr();
  Node* currptr = getHeadPtr();
  ...
  if (size() == 0) {
    ...
    for (int i = 0; i < size(); i++) {
      if (currptr->data > newData) {
        if (currptr == getHeadPtr()) {
          // current data is the first one in list
          pushFront(newData);
        } 
        else {
          // copy the list to a backup list
          LinkedList<T> backup = *this;
          // create a new pointer 
          Node* newPointerToUse = currptr->prev;
          // get the data
          int value = currptr->data;
          // pop up to and including the current pointer value
          while (getTailPtr() != newPointerToUse) {
            popBack();
          } 
          // push back the new node and everything that has been popped (refer to backup)
          pushBack(newData);
          Node* backupHeadptr = backup.getHeadPtr();
          for (int i = 0; i < backup.size(); i++) {           
            if (backupHeadptr->data >= value) {
              pushBack(backupHeadptr->data);
            }
            backupHeadptr = backupHeadptr->next;
          }
        }
        return;
      } 
      else if (currptr == tailptrBeforeHack) {
        // current pointer's data is smaller than the new data, AND current pointer is equal to tail pointer
        pushBack(newData);
        return;
      }
  ...
  // then walk the list from front to back and find
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  int numLoops = left.size() + right.size();
  for (int i = 0; i < numLoops; i++) {
    Node* ptrLeft = left.getHeadPtr();
    Node* ptrRight = right.getHeadPtr();

    if (ptrLeft == nullptr && ptrRight == nullptr) {
      // both null, so merged has finished
      return merged;
    } 
    else if (ptrLeft == nullptr) {
      // add right to merged
      merged.pushBack(ptrRight->data);
      right.popFront();
    }
    else if (ptrRight == nullptr) {
      // add left to merged
      ...
      else if (ptrRight->data > ptrLeft->data) {
        // left is smaller
        merged.pushBack(ptrLeft->data);
        left.popFront();
      }
